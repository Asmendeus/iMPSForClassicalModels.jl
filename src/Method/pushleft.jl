...
function pushleft(X₀::LeftEnvironmentTensor{N}, A::LocalTensor{R}, O::AbstractVector{MPOTensor}, B::AdjointLocalTensor{R}) where {N, R}
    ...
    elseif R == 3 && N == 3
        @tensor X[-1; -2 -3] := X₀.A[1 2 3] * A.A[3 5 -3] * O[1].A[2 4 5 -2] * B.A[-1 1 4]
        return typeof(X₀)(X)
    elseif R == 4 && N == 3
        @tensor X[-1; -2 -3] := X₀.A[1 2 3] * A.A[3 5 6 -3] * O[1].A[2 4 5 -2] * B.A[6 -1 1 4]
        return typeof(X₀)(X)
    elseif R == 3 && N == 4
        @tensor X[-1; -2 -3 -4] := X₀.A[1 2 3 4] * A.A[4 7 -4] * O[1].A[3 6 7 -3] * O[2].A[2 5 6 -2] * B.A[-1 1 5]
        return typeof(X₀)(X)
    elseif R == 4 && N == 4
        @tensor X[-1; -2 -3 -4] := X₀.A[1 2 3 4] * A.A[4 7 8 -4] * O[1].A[3 6 7 -3] * O[2].A[2 5 6 -2] * B.A[8 -1 1 5]
        return typeof(X₀)(X)
    else
        throw(ArgumentError("Undefined behavior. You can add your requirements to the function based on the comments in the source code at the error location."))
        # ===== Generate the code you need =====
        ...
        # str_O = prod(["* O[$w].A[$(N-w) $(2*N-w-1) $(2*N-w) -$(N-w)] " for w in 1:N-2])
...
function pushleft(X₀::LeftEnvironmentTensor{N}, env::ChannelEnvironment{N, L, R}) where {N, L, R}
    return pushleft(X₀, env.A, env.O, env.B)
end
