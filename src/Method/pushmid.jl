...
Contract bond tensor and bond environment
    ...
    |  | --   --- |  |
     ‾‾            ‾‾
"""
function pushmid(X₀::BondTensor, FL::LeftEnvironmentTensor{2}, FR::RightEnvironmentTensor{2})
    @tensor X[-1; -2] := FL.A[-1 1] * X₀.A[1 2] * FR.A[2 -2]
    return typeof(X₀)(X)
end
function pushmid(X₀::BondTensor, env::BondEnvironment{2})
...
Contract adjoint bond tensor and bond environment
    ...
    |  | -- X₀ -- |  |
     ‾‾            ‾‾
"""
function pushmid(X₀::AdjointBondTensor, FL::LeftEnvironmentTensor{2}, FR::RightEnvironmentTensor{2})
    @tensor X[-1; -2] := FL.A[2 -2] * X₀.A[1 2] * FR.A[-1 1]
    return typeof(X₀)(X)
end
function pushmid(X₀::AdjointBondTensor, env::BondEnvironment{2})
    return pushmid(X₀, env.FL, env.FR)
end
...
"""
    pushmid(X₀::LocalTensor{R}, FL::LeftEnvironmentTensor{2}, FR::RightEnvironmentTensor{2})
        -> X::LocalTensor{R}
    pushmid(X₀::LocalTensor{R}, env::BondEnvironment{2})
        -> X::LocalTensor{R}
...
Contract local tensor and center environment
    ...
    |  | --   --- |  |        |
     ‾‾            ‾‾
"""
function pushmid(X₀::LocalTensor{R}, FL::LeftEnvironmentTensor{2}, FR::RightEnvironmentTensor{2}) where R
    if R == 3
        @tensor X[-1 -2; -3] := FL.A[-1 1] * X₀.A[1 -2 2] * FR.A[2 -3]
        return typeof(X₀)(X)
    elseif R == 4
        @tensor X[-1 -2; -3 -4] := FL.A[-1 1] * X₀.A[1 -2 -3 2] * FR.A[2 -4]
        return typeof(X₀)(X)
    else
        throw(ArgumentError("Undefined behavior. You can add your requirements to the function based on the comments in the source code at the error location."))
        # ===== Generate the code you need =====
        ...
        # str_legs_X₀ = "1" * prod([" -$i" for i in 2:R-1]) * " 2"
        # str_expr = "@tensor X[$str_legs_X] := FL.A[-1 1] * X₀.A[$str_legs_X₀] * FR.A[2 -$R]"
        # ==========
        # Dev memo: No method was found for `eval` to be applied to function scopes
    end
end
function pushmid(X₀::LocalTensor{R}, env::BondEnvironment{2}) where R
    return pushmid(X₀, env.FL, env.FR)
end

"""
    pushmid(X₀::AdjointLocalTensor{R}, FL::LeftEnvironmentTensor{2}, FR::RightEnvironmentTensor{2})
        -> X::AdjointLocalTensor{R}
    pushmid(X₀::AdjointLocalTensor{R}, env::BondEnvironment{2})
        -> X::AdjointLocalTensor{R}
...
Contract local tensor and center environment
    ...
    |  | -- X₀ -- |  |        |
     ‾‾     |      ‾‾        (a)
           (a)
"""
function pushmid(X₀::AdjointLocalTensor{R}, FL::LeftEnvironmentTensor{2}, FR::RightEnvironmentTensor{2}) where R
    if R == 3
        @tensor X[-1; -2 -3] := FL.A[2 -2] * X₀.A[1 2 -3] * FR.A[-1 1]
        return typeof(X₀)(X)
    elseif R == 4
        @tensor X[-1 -2; -3 -4] := FL.A[2 -3] * X₀.A[-1 1 2 -4] * FR.A[-2 1]
        return typeof(X₀)(X)
    else
        throw(ArgumentError("Undefined behavior. You can add your requirements to the function based on the comments in the source code at the error location."))
        # ===== Generate the code you need =====
        ...
        # str_legs_X₀ = prod(["-$i " for i in 1:R-3]) * "1 2 -$R"
        # str_expr = "@tensor X[$str_legs_X] := FL.A[2 -$(R-1)] * X₀.A[$str_legs_X₀] * FR.A[-$(R-2) 1]"
        # ==========
        # Dev memo: No method was found for `eval` to be applied to function scopes
    end
end
function pushmid(X₀::AdjointLocalTensor{R}, env::BondEnvironment{2}) where R
    return pushmid(X₀, env.FL, env.FR)
end
